{
  ...
  nixConfig.bash-prompt = "\\033[0;33m\\033[1m\[dev-golden-rust\] \\w\\033[0m\\033[0m$ ";
  ...
  outputs = { self, nixpkgs, rust-overlay }:
    ...
    in
    {
      ...
      overlay = final: prev: {
        golden-rust = with final; prev.callPackage ./derivation.nix {
          src = self;
        };
        golden-rust-nightly = prev.callPackage ./derivation.nix {
          rustPlatform = let rustSpecific = final.rust-bin.nightly.latest.minimal; in prev.makeRustPlatform { rustc = rustSpecific; cargo = rustSpecific; };
          src = self;
        };
      };
